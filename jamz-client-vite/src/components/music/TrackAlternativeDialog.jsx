...
const TrackAlternativeDialog = ({ 
  ...
  const handleSearch = async (customSearchTerm = null) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || err.message || 'Failed to search for alternatives';
      setError(`${errorMsg}. You can manually paste a YouTube video ID below.`);
    } finally {
      setSearching(false);
    }
  };
